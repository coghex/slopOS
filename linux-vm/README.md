...
- `rootfs/bootstrap-manifest.toml` now also names the minimum `buildroot_seed_surface.critical_paths` that the extracted base archive must provide, plus the stage0 paths such as `/usr/bin/wget`, `/usr/bin/file`, `/usr/bin/make`, `/usr/bin/patch`, and `/usr/bin/perl` that are now intentionally absent before the guest helper is allowed to apply repo-owned normal-rootfs assembly
